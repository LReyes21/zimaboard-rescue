...
read -r -p "1) Ensure USB-C ethernet is connected and interface is up. Press ENTER when ready..." || true
ip -brief link | grep -E 'enx|eth' || true

read -r -p "2) (Optional) Start DHCP rescue on interface (runs dnsmasq). Enter interface name or leave blank to skip: " IFACE
if [[ -n "$IFACE" ]]; then
  sudo bash -c "$SCRIPTS/rescue_dhcp.sh $IFACE"
  echo "Started rescue DHCP (foreground). Ctrl+C to stop and continue checklist."
  read -r -p "Press ENTER after you stopped dnsmasq to continue..." || true
fi

read -r -p "3) Try SSH to last-known host (userver.local / 192.168.0.147). Press ENTER to run ping..." || true
ping -c 3 userver.local || ping -c 3 192.168.0.147 || true

read -r -p "4) Collect non-privileged diagnostics? [y/N] " yn
if [[ "$yn" =~ ^[Yy] ]]; then
...
read -r -p "5) Record this checklist run to DB? [y/N] " yn2
if [[ "$yn2" =~ ^[Yy] ]]; then
  read -r -p "Summary for record: " summary
